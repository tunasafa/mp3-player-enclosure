...
files=sorted(p for p in ROOT.rglob('*') if p.is_file() and p.suffix not in ['.zip','.pyc'] and '__pycache__' not in p.parts and p.name!='manifest.sha256.json')
manifest={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in files}
mf=ROOT/'manifest.sha256.json';mf.write_text(json.dumps(manifest,indent=2)+'\n')
dependencies=list(v['dependency_sha256'])+['revision_04/vendor/README.md','revision_04/vendor/LICENSE-viewer.txt','revision_04/requirements.txt','revision_04/package.json','revision_04/package-lock.json','touchscreen_01/preview.html']
archive=ROOT/'Model02_M02-02_MECHANICAL_PROTOTYPE.zip'
