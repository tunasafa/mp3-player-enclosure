"""Dimensioned drawings from validated T03 bounds, not hand-positioned boxes."""
...
y=.95
for title,text in [
 ('8.3 mm aggressive prototype','0.4 mm steel front / 0.15 mm insulation\n7.1 mm full Adafruit product height\n0.18 mm clearance + 0.07 mm liner\n0.4 mm steel rear'),
 ('Separate battery bay','5 mm nominal protected pack\n2.13 mm to the rear liner\n1.7 mm of that reserved clear'),
 ...
 y-=.245 if title.startswith('8.3') else .18
fig.text(.05,.94,'mytunas / touch',fontsize=27,color=ink)
fig.text(.05,.90,f'T03 / BATTERY ABOVE DAC / BOTTOM JACK / {w} × {l} × {t} mm',fontsize=11,color=muted)
fig.text(.05,.075,'Metal faces + separate frame. Same coordinates as STEP and viewer; dimensions in millimetres.',fontsize=9,color=muted)
fig.text(.05,.05,'Not a fabrication release: qualify plate flatness, deflection, insulation, tolerances and physical component fit.',fontsize=9,color=muted)
for ext in ['png','pdf']:fig.savefig(ROOT/f'design_overview.{ext}',dpi=170,facecolor=fig.get_facecolor())
print('Wrote T03 dimensioned drawings')
